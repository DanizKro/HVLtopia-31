...
#Generate random data for a year
		def
GenereateRandomYearDataList(intencity:float, seed:int = 0)->list[int]:
if seed
!=0:	
		np.random.seed(seed) 
...
#NOx Data
		...
		axNox.set_title("NOX nivåer" if quarterYear == 0
				else
				f "NOx Nivåer - Kvartal {quarterYear}")
...
#Asphalt Dust Data
					...
					axAsfalt.set_title("Asfaltlstøv nivåer" if quarterYear == 0
							   else
							   f "Asfaltstøv verdier - Kvartal {quarterYear}")
